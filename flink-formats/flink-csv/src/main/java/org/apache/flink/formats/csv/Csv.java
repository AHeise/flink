...
public class Csv<T, SELF extends Csv<T, SELF>> extends AbstractConfigurable<SELF> implements
    ...
    public static class UntypedCsv<T> extends Csv<T, UntypedCsv<T>> {
        ...
        private UntypedCsv(
                CsvMapper mapper,
                CsvSchema schema, Configuration configuration) {
            super(mapper, schema, configuration);
        }
    }

    public static class TypedCsv<T> extends Csv<T, TypedCsv<T>> implements
            StreamReadable<T, TypedCsv<T>> {

        private final TypeInformation<T> typeInformation;

        public TypedCsv(
                CsvMapper mapper,
                CsvSchema schema, Configuration configuration, TypeInformation<T> typeInformation) {
            super(mapper, schema, configuration);
