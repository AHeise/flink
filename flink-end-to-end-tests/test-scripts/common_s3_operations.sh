...
function s3_get_by_full_path_and_filename_prefix {
  ...
  aws_cli s3 cp "s3://$IT_CASE_S3_BUCKET/$2" "/hostdir/${1#$TEST_INFRA_DIR}"$args
}
...
function s3_delete_by_full_path_prefix {
  aws_cli s3 rm "s3://$IT_CASE_S3_BUCKET/$1" --recursive
}
