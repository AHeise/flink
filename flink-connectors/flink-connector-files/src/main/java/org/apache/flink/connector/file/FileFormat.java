...
public interface FileFormat<T, SELF extends FileFormat<T, SELF>> extends Format<T, SELF> {
     ...
     */
    interface StreamReadable<T, SELF extends StreamReadable<T, SELF>> extends
            WithTypeInformation<T, SELF>, FileFormat<T, SELF> {
     ...
     */
    interface BulkReadable<T, SELF extends BulkReadable<T, SELF>> extends
            WithTypeInformation<T, SELF>, FileFormat<T, SELF> {
    ...
    default Set<ConfigOption<?>> forwardOptions() {
        return Stream.concat(requiredOptions().stream(), optionalOptions().stream()).collect(
                Collectors.toSet());
    }
}
