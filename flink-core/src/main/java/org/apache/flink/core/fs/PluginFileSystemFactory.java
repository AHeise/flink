...
public class PluginFileSystemFactory implements FileSystemFactory {
	...
	private PluginFileSystemFactory(final FileSystemFactory inner) {
		this.inner = inner;
	}

	public static PluginFileSystemFactory of(final FileSystemFactory inner) {
		return new PluginFileSystemFactory(inner);
	}
	...
	public FileSystem create(final URI fsUri) throws IOException {
		return ClassLoaderFixingFileSystem.of(inner.create(fsUri));
	}
	...
	private static class ClassLoaderFixingFileSystem extends FileSystem {
		...
		private ClassLoaderFixingFileSystem(final FileSystem inner) {
			this.inner = inner;
			loader = inner.getClass().getClassLoader();
		}

		private static ClassLoaderFixingFileSystem of(final FileSystem inner) {
			return new ClassLoaderFixingFileSystem(inner);
		}
