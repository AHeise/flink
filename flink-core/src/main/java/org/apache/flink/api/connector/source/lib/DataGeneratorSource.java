...
public class DataGeneratorSource<T>
    ...
    public final MapFunction<Long, T> generatorFunction;
    ...
    public DataGeneratorSource(
            TypeInformation<T> typeInfo, MapFunction<Long, T> generatorFunction, long count) {
    ...
    @Override
    public SourceReader<T, GeneratorSequenceSplit<T>> createReader(
            SourceReaderContext readerContext) {
        return new IteratorSourceReader<>(readerContext);
    }

    @Override
    public SplitEnumerator<GeneratorSequenceSplit<T>, Collection<GeneratorSequenceSplit<T>>>
            createEnumerator(final SplitEnumeratorContext<GeneratorSequenceSplit<T>> enumContext) {
    ...
    public SplitEnumerator<GeneratorSequenceSplit<T>, Collection<GeneratorSequenceSplit<T>>>
            restoreEnumerator(
                    final SplitEnumeratorContext<GeneratorSequenceSplit<T>> enumContext,
                    Collection<GeneratorSequenceSplit<T>> checkpoint) {
        return new IteratorSourceEnumerator<>(enumContext, checkpoint);
    }

    @Override
    public SimpleVersionedSerializer<GeneratorSequenceSplit<T>> getSplitSerializer() {
        return new SplitSerializer<>(numberSource.getSplitSerializer(), generatorFunction);
    }

    @Override
    public SimpleVersionedSerializer<Collection<GeneratorSequenceSplit<T>>>
